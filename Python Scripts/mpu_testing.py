...
def concatenate(gyro,acc,combined):
    result = "gyro:"
    result += "X=" + str(gyro[0][0]) + ","
    result += "Y=" + str(gyro[0][1]) + ","
    result += "Z=" + str(gyro[0][2]) + ";"
    result += "acc:"
    result += "X=" + str(acc[0][0]) + ","
    result += "Y=" + str(acc[0][1]) + ","
    result += "Z=" + str(acc[0][2]) + ";"
    result += "combined:"
    result += "X=" + str(combined[0][0]) + ","
    result += "Y=" + str(combined[0][1]) + ","
    result += "Z=" + str(combined[0][2]) + ";"
    return result
...
if __name__ == "__main__":
    try:
        ...
        for i in range(NR_OF_DEVICES):
            ...
            mpu[i] = MPU9250(bus=spi[i],gfs=0x00,afs=0x01)
        ...
        while True:
            ...
            for i in range(NR_OF_DEVICES):
                ...
                # PRINT
                # print("EULER ANGLES: ",euler[i])

            # PUBLISH + LOOP RATE
            # print(flt_gyro_angles[0])
            msg = concatenate(flt_gyro_angles,flt_acc_angles,euler)
